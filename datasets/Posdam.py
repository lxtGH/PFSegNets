...
def make_dataset(quality, mode, hardnm=0):
    ...
    logging.info(f'Posdam896 has a total of {len(all_tokens)} images in {mode} phase')

    logging.info(f'Posdam-{mode}: {len(all_tokens)} images')

    return all_tokens


class POSDAMDataset(data.Dataset):
