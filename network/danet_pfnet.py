...
class PointFlowDecoder(nn.Module):
    ...
    def __init__(self, num_class, norm_layer=nn.BatchNorm2d, fpn_inplanes=[256, 512, 1024, 2048], fpn_dim=128,
                 fpn_dsn=False, reduce_dim=32, max_pool_size=8, avgpool_size=8, edge_points=32):
        super(PointFlowDecoder, self).__init__()
        ...
        for i in range(len(fpn_inplanes) - 1):
            ...
            self.fpn_out_align.append(PointFlowModuleWithMaxAvgpool(fpn_dim, dim=reduce_dim, maxpool_size=max_pool_size,
...
class DANet(nn.Module):
    ...
    def __init__(self, num_classes, trunk='seresnext-50', criterion=None, variant="D",
                 reduce_dim=32, max_pool_size=8, avgpool_size=8, edge_points=32):
        ...
        self.point_flow_decoder = PointFlowDecoder(num_classes, fpn_inplanes=[256, 512, 1024, 512], norm_layer=Norm2d,
                                                   reduce_dim=reduce_dim, max_pool_size=max_pool_size,
                                                   avgpool_size=avgpool_size, edge_points=edge_points)
